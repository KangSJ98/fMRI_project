function drawCard(card, center_x, center_y, opt)
%% 색상 설정
switch card.color
    case 'red'
        color = [255 0 0]; % 빨간색
...
switch card.shape
    ...
    case 'triangle'
        ...
        if strcmp('outline', card.shadow)
            ...
            Screen('FillPoly', opt.window, opt.backgroundColor, [outline_x; outline_y]', 1);
        end
        % 사각형
    case 'square'
        % 정사각형의 세 꼭지점 좌표 계산
    ...
    case 'circle'
        ...
        if strcmp('outline', card.shadow)
            Screen('FillOval', opt.window, opt.backgroundColor, [center_x - opt.radius * 0.9, center_y - opt.radius * 0.9, center_x + opt.radius * 0.9, center_y + opt.radius * 0.9]);
        end
end
...
if strcmp(card.shadow, 'stripe')
    ...
    for i = 2:2:20
        ...
        % 영역 채우기
        Screen('FillRect', opt.window, opt.backgroundColor, [rect_left, stripe_top, rect_right, stripe_bottom]);
    end
end
