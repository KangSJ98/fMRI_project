function answerCard = CalculateAnswerCard(card1, card2, opt)
    ...
    if strcmp(card1.shape, card2.shape)
        ...
        shape_candidates = opt.shapes(~ismember(opt.shapes, {card1.shape, card2.shape}));
    ...
    if strcmp(card1.color, card2.color)
        ...
        color_candidates = opt.colors(~ismember(opt.colors, {card1.color, card2.color}));
        answerCard.color = color_candidates{randi(length(color_candidates))};
    end
    
    % 개수 결정
    if strcmp(card1.number, card2.number)
        answerCard.number = card1.number;
    else
        number_candidates = opt.numbers(~ismember(opt.numbers, {card1.number, card2.number}));
    ...
    if strcmp(card1.shadow, card2.shadow)
        ...
        shadow_candidates = opt.shadows(~ismember(opt.shadows, {card1.shadow, card2.shadow}));
